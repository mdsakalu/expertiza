#Signup controller has all the functions related to signing up for and dropping a selected
...
#people who have already chosen the topic, etc
  def signup_topics
    ...
    @signup_topics = SignupTopic.find(:all, :conditions => ['assignment_id = ?', params[:id]])
...
#checks for other topics a user may have already signed up for. These include both confirmed as well as waitlisted topics
  ...
  def confirmTopic(creator_id, topic_id, assignment_id)
    ...
    signup = SignedUpUser.new
    signup.topic_id = params[:id]
    signup.creator_id = creator_id
