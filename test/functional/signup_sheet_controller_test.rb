...
class SignupSheetControllerTest < ActionController::TestCase
  ...
  test "should_delete_signup_topic_for_assignment" do
    post :delete, :id=> signup_topics(:topic1).id, :assignment_id => assignments(:assignment2)
    newTopic = SignupTopic.find_by_assignment_id(assignments(:assignment2).id)
    assert_nil newTopic
  end

  test "should_be_able_to_edit_topics" do
    post :update, :topic =>{:topic_identifier=>"t1",:topic_name=>"t1",:category=>"t1",:max_choosers=>3}, :assignment_id=>assignments(:assignment2).id, :id=>signup_topics(:topic1).id
    assert_equal "t1", signup_topics(:topic1).topic_name
  end
